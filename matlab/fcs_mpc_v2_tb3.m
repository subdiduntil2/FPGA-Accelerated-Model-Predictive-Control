%% Run Testbench: ROBUST <1m TRACKING (Safety Constraints)
%
%  Single source of truth for everything downstream:
%
%   - 8 .dat files (x.dat, y.dat, psi.dat, v.dat, ref_x.dat, ref_y.dat,
%     accel_cmd_expected.dat, steer_cmd_expected.dat)
%       -> kept for legacy compatibility, but the new VHDL TB no longer
%          reads them.
%
%   - trajectory_data.h    -> compiled into the Cora bare-metal C app.
%   - trajectory_data_pkg.vhd
%       -> consumed by fcs_mpc_v2_fixpt_tb.vhd at *elaboration* time
%          (constant arrays in a VHDL package).  No std.textio, no file
%          I/O at simulation time -- the data is part of the design.
%
%  All three artefacts come from the same in-memory hist_* arrays, so
%  there is no possibility of drift between MATLAB sim, VHDL sim, and
%  FPGA replay.
clear; clc; close all;
...
% --- WRITE TRAJECTORY DATA (all three formats) -------------------------
out_dir = pwd;
...
function write_trajectory_files(out_dir, x, y, psi, v, rx, ry, eacc, estr)
    ...
    % --- 2. trajectory_data.h (C, for Cora) ---
    h_specs = { ...
    ...
    % --- 3. trajectory_data_pkg.vhd (VHDL package, for the new VHDL TB) ---
    %
    % Emits a package with TRAJECTORY_LEN, an int_array type, and an array
    % per signal.  The TB reads these as integers and casts to signed/unsigned
    % of the right port width.  Using INTEGER avoids any width/sign confusion
